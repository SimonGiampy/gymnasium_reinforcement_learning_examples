...
MAX_STEPS = 15  # max steps per episode before termination


def setup_env_training(base_env: EnvBase, num_envs: int):
    ...
    env = TransformedEnv(
        ...
        transform=Compose(  # a sequence of transforms to apply
            CatTensors(in_keys=["joint_angles", "head_height", "extremities", "torso_vertical", "com_velocity", "velocity"],
                       out_key="observation"),
            norm_transform,
    ...
    norm_transform.init_stats(num_iter=5*MAX_STEPS,  # number of iterations to run for normalization statistics computation
                              reduce_dim=0, cat_dim=0)  # dimension axis to reduce and concatenate on (tensors axis)
...
def setup_env_inference(base_env: EnvBase):
    ...
    cat_tensors_module = CatTensors(
        in_keys=["joint_angles", "head_height", "extremities", "torso_vertical", "com_velocity", "velocity"],
        out_key="observation"
    )
