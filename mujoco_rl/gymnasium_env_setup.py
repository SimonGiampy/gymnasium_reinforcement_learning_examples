...
def setup_env_training(base_env: EnvBase, batch_dim: int = 0):
    ...
    norm_transform.init_stats(num_iter=MAX_STEPS,  # number of iterations to run for normalization statistics computation
                              reduce_dim=batch_dim, cat_dim=batch_dim)  # dimension axis to reduce and concatenate on (data axis)
...
def setup_parallel_env(base_env: EnvBase, num_envs: int):
    ...
    # create parallel environment wrapper for base environment
    parallel_env = ParallelEnv(
        num_workers=num_envs,
        create_env_fn=lambda: base_env
    )
    
    # axis 0 is the batch dimension for parallel environments
    # axis 1 is the axis where to compute normalization statistics
    return setup_env_training(parallel_env, batch_dim=1)
