...
# https://github.com/google-deepmind/dm_control/blob/main/dm_control/suite/

num_envs = 32  # number of parallel environments for training

def training(env_name: str, task_name: str):
    model_file_name = f"models/{env_name}_{task_name}_deepmind_mujoco"
    ...
    mj_ppo.set_lr(min_learning_rate=5e-5, max_learning_rate=5e-4)
    ...
    # create environment for inference without batching
    ...
    # run inference with the trained policy for default max steps
    run_inference_rendered(env_inference, trained_actor_policy)

    env.close()

def inference(env_name: str, task_name: str):
    model_file_name = f"models/{env_name}_{task_name}_deepmind_mujoco"
...
def select_environment():
    print("Available DeepMind Control environments: ", DMControlEnv.available_envs)
    env_name = input("Select robot environment to run (e.g., humanoid, quadruped, ...): ")
    task_name = input("Select task for the environment (e.g., run, walk, ...): ")
    return env_name, task_name

def main():
    env_name, task_name = select_environment()
    # if run with --train argument, perform training; otherwise, run inference
    if "--train" in sys.argv:
        training(env_name, task_name)
    elif "--inference" in sys.argv:
        inference(env_name, task_name)
    else:
